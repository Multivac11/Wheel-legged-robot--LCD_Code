...
DMA2D_HandleTypeDef hdma2d_f;	// DMA2D句柄
LTDC_HandleTypeDef hltdc_f;		// LTDC句柄
...
void SystemClock_Config_f(void)
{
...
void MX_LTDC_Init_F(void)
{
	LTDC_LayerCfgTypeDef pLayerCfg = {0};		// layer0 相关参数
	__HAL_RCC_DMA2D_CLK_ENABLE();					// 使能DMA2D时钟

	hltdc_f.Instance 		 = LTDC;
	hltdc_f.Init.HSPolarity = LTDC_HSPOLARITY_AL;		// 低电平有效
	hltdc_f.Init.VSPolarity = LTDC_VSPOLARITY_AL;     // 低电平有效
	hltdc_f.Init.DEPolarity = LTDC_DEPOLARITY_AL;     // 低电平有效，要注意的是，很多面板都是高电平有效，但是750需要设置成低电平才能正常显示
	hltdc_f.Init.PCPolarity = LTDC_PCPOLARITY_IPC;    // 正常时钟信号

	hltdc_f.Init.HorizontalSync 		= HSW - 1;									// 根据屏幕设置参数即可
	hltdc_f.Init.VerticalSync 		= VSW	-1 ;
	hltdc_f.Init.AccumulatedHBP		= HBP + HSW -1;
	hltdc_f.Init.AccumulatedVBP 		= VBP + VSW -1;
	hltdc_f.Init.AccumulatedActiveW = LCD_Width  + HSW + HBP -1;
	hltdc_f.Init.AccumulatedActiveH = LCD_Height + VSW + VBP -1;
	hltdc_f.Init.TotalWidth 			= LCD_Width  + HSW + HBP + HFP - 1;
	hltdc_f.Init.TotalHeigh 			= LCD_Height + VSW + VBP + VFP - 1;

	hltdc_f.Init.Backcolor.Red 		= 0;	// 初始背景色，R
	hltdc_f.Init.Backcolor.Green 	= 0;  // 初始背景色，G
	hltdc_f.Init.Backcolor.Blue 		= 0;	// 初始背景色，B

	HAL_LTDC_Init(&hltdc_f);	// 初始化LTDC参数
...
// 配置 layer0 的初始默认颜色，包括A,R,G,B 的值 ，最终写入 LTDC_LxDCCR 寄存器
	...
	HAL_LTDC_ConfigLayer(&hltdc_f, &pLayerCfg, 0);		// 配置layer0
...
// 配置layer1 的初始默认颜色，包括A,R,G,B 的值 ，最终写入 LTDC_LxDCCR 寄存器
	...
	HAL_LTDC_ConfigLayer(&hltdc_f, &pLayerCfg1, 1);	// 初始化 layer1 的配置
