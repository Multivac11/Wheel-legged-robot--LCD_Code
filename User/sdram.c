...
SDRAM_HandleTypeDef hsdram1_f;			// SDRAM_HandleTypeDef 结构体变量
...
void HAL_SDRAM_MspInit_F(SDRAM_HandleTypeDef* hsdram)
{
	HAL_FMC_MspInit();
}
...
void MX_FMC_Init_F(void)
{
	FMC_SDRAM_TimingTypeDef SdramTiming = {0};

	hsdram1_f.Instance = FMC_SDRAM_DEVICE;
	/* hsdram1.Init */
	hsdram1_f.Init.SDBank					 	= FMC_SDRAM_BANK1;							// 选择BANK区
	hsdram1_f.Init.ColumnBitsNumber 		= FMC_SDRAM_COLUMN_BITS_NUM_9;         // 行地址宽度
	hsdram1_f.Init.RowBitsNumber 			= FMC_SDRAM_ROW_BITS_NUM_13;           // 列地址线宽度
	hsdram1_f.Init.MemoryDataWidth 			= FMC_SDRAM_MEM_BUS_WIDTH_16;          // 数据宽度
	hsdram1_f.Init.InternalBankNumber 		= FMC_SDRAM_INTERN_BANKS_NUM_4;        // bank数量
	hsdram1_f.Init.CASLatency 				= FMC_SDRAM_CAS_LATENCY_3;             //	CAS
	hsdram1_f.Init.WriteProtection 			= FMC_SDRAM_WRITE_PROTECTION_DISABLE;  // 禁止写保护
	hsdram1_f.Init.SDClockPeriod 			= FMC_SDRAM_CLOCK_PERIOD_2;            // 分频
	hsdram1_f.Init.ReadBurst 					= FMC_SDRAM_RBURST_ENABLE;             // 突发模式
	hsdram1_f.Init.ReadPipeDelay 			= FMC_SDRAM_RPIPE_DELAY_1;             // 读延迟
	...
	HAL_SDRAM_Init(&hsdram1_f, &SdramTiming);	// 初始化FMC接口
									
	SDRAM_Initialization_Sequence(&hsdram1_f,&command);//配置SDRAM
}
